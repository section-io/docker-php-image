...
RUN apt-get update && apt-get install -y ssmtp bash vim curl git curl patch gzip
ADD requireme.php .
ADD test.php .
...
# Install xml libs
#RUN set -ex && ( \
#  export CFLAGS="-I/usr/src/php" && apt-get install -y php7.4-xml php7.4-xsl \
#)
...
#    --with-jpeg \
#RUN set -ex && ( \
#  apt-get install -y intl zip pdo_mysql mysqli \
#)
